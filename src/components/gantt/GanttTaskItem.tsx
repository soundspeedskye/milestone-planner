...
export const GanttTaskItem = memo(function GanttTaskItem({
  task, index, roles, warnings, dragging, over, onOver,
}: Props) {
  ...
  return (
    ...
    >
      ...
      <span className="fixed-start" title="시작일 고정: 지정하면 순차 계산 대신 이 날짜부터 시작해요">
        ...
        />
        {task.fixedStart && (
          <button className="btn-unpin" title="고정 해제" onClick={() => setFixedStart(task.id, undefined)}>✕</button>
        )}
      </span>
      <div className="gantt-task-days">
        {roles.filter(r => (task.days[r.id] || 0) > 0).map(r => (
          <span
            key={r.id}
            className="day-badge"
      ...
      <button className="btn-eject" onClick={() => ejectFromGantt(task.id)} title="보관함으로 꺼내기">↩</button>
    </div>
  )
})
