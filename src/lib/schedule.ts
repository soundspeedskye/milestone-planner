...
export function calcSchedules(
  ...
  roles.forEach(r => { roleEnd[r.id] = new Date(projectStart) })
  const roleById = new Map(roles.map(r => [r.id, r]))

  return tasks.map(task => {
    const s: TaskSchedule = { id: task.id, name: task.name || '(무제)', roles: {}, warnings: [] }
    ...
    const effectiveEnd = (roleId: string): Date => {
      ...
      if (!role || computing.has(roleId)) return roleEnd[roleId] ?? new Date(projectStart)
      ...
      if (days > 0) {
        ...
        const depMax = depEnds.length ? Math.max(...depEnds.map(d => d.getTime())) : 0
        ...
        if (fixedStart) {
          ...
          if (fixedT < roleEnd[roleId].getTime()) {
            s.warnings.push(`${role.name}: 고정 시작일이 앞 일정(${fmt(roleEnd[roleId])} 종료)과 겹쳐요`)
          }
          startT = fixedT
        }
        const start = new Date(startT)
        const end = addWD(start, days, isWD)
...
export function scheduleRange(schedules: TaskSchedule[]): { min: Date; max: Date } | null {
  const starts: number[] = []
  const ends: number[] = []
  schedules.forEach(s => Object.values(s.roles).forEach(r => {
